#include <stdint.h>  // Para os tipos uint8_t e uint32_t
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        printf("Uso: %s <texto_para_hash>\n", argv[0]);
        return 1;
    }

    // Exemplo de uso da função ripemd160 com o argumento da linha de comando
    const uint8_t* data = (const uint8_t*)argv[1];  // Pega a string passada pela linha de comando
    uint32_t data_len = strlen(argv[1]);  // Tamanho da string de entrada
    uint8_t digest[20];  // Buffer para o hash de saída (RIPEMD-160 gera 20 bytes)

    ripemd160(data, data_len, digest);
